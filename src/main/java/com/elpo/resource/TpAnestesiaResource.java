...
/**
 * Recurso de Comorbidade caminho de acesso: http://localhost:8080/tpanestesias
...
@RequestMapping("/tpanestesias")
public class TpAnestesiaResource {

	@Autowired
	private TpAnestesiaRepository dao;
	
	@Autowired
	private ApplicationEventPublisher eventPublisher;

	/**
	 * http://localhost:8080/tpanestesias
	 * 200 ok
	 * @return
	 */
	@GetMapping
	...
	/**
     * http://localhost:8080/tpanestesias/{id}
     * @param codigo
     * @return 
     */
    @GetMapping("/{id}")
    public ResponseEntity<TpAnestesia> buscarPorCodigo(@PathVariable Long id){
        System.out.println("Entrei no método @GetMapping({id})");
        Optional<TpAnestesia> obj = dao.findById(id);
	...
	/**
	 * Configurado somente para Criar http://localhost:8080/tpanestesias
	 * 201 created
	 * @param obj
	 * @return
	 */
	@PostMapping
	public ResponseEntity<TpAnestesia> criar(@Valid @RequestBody TpAnestesia obj, HttpServletResponse response) {
		System.out.println("Entrei no método @PostMapping");
		TpAnestesia objSalvo = dao.save(obj);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TpAnestesia> atualizar(@PathVariable Long id, @Valid @RequestBody TpAnestesia obj){
    	System.out.println("Entrei no método @PutMapping(/{id})" + id);
    	Optional<TpAnestesia> objOpt = dao.findById(id);
    	TpAnestesia objSalvo = objOpt.get();
